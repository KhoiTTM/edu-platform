...
7. Ví dụ tham khảo: `content/khtn7-questions.json` (91 câu, Bài 1-5 sách SBT KHTN 7) và `content/khtn7-answer-key.json` (đáp án thô theo `Bài.Câu`, dùng tiếp khi OCR thêm bài). Tên bài hiển thị khai báo trong `LESSON_TITLES` (cả 2 file route) — **bắt buộc thêm dòng mới khi OCR bài tiếp theo**, nếu không trang danh sách Bài chỉ hiện "Bài N" trống.
8. **Hạn chế đã biết:** OCR đôi khi đánh rơi hẳn 1 số thứ tự (ví dụ mất số "17" trong "2.17"), khiến nội dung 2 câu liền kề bị dính vào nhau — không sửa được an toàn bằng regex, cần review tay. Cũng có thể gặp case số/dấu chấm ngay trước marker thật bị lookbehind nuốt nhầm (đã gặp ở Bài 5, câu 5.4 và 5.15) — sửa bằng cách patch thủ công xoá cụm gây nhiễu trong text trước khi parse, xác minh đúng vị trí qua ảnh gốc trước khi patch.
