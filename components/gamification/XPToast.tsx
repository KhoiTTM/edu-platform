    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="pointer-events-none bg-amber-900/60 border-2 border-amber-800 rounded-2xl px-4 py-2 flex items-center gap-2 shadow-xl backdrop-blur-sm z-[100]"
        >
          <div className="bg-amber-500 rounded-full p-1 shadow-inner">
            <span className="text-white font-black text-[10px]">XP</span>
          </div>
          <span className="text-amber-400 font-black text-lg">+{xp}</span>
        </motion.div>
      )}
    </AnimatePresence>
  );
