...
async function seed() {
  console.log("Starting static seeding for Vietnamese 3 Theme 1...");
  ...
  if (!subjectNode) {
    const { data: newSubj, error: subjErr } = await supabase.from('curriculum_nodes').insert({
      title: 'Tiếng Việt 3',
      slug: 'tieng_viet',
      type: 'subject',
      source_id: sourceId,
      path: 'tieng_viet',
      depth: 0,
      metadata: { color: 'text-rose-400', icon: 'BookOpen' }
    }).select().single();
  ...
  const unitTitle = data.meta.topic || "Chủ điểm 1: Những trải nghiệm thú vị";
  const unitSlug = "chu_diem_1";
  ...
  for (const examData of data.exams) {
    ...
    // Create Collection
    const { data: collection, error: colErr } = await supabase.from('assessment_collections').insert({
    ...
    const { data: exam, error: exErr } = await supabase.from('exams').insert({
      ...
      metadata_json: { lessons: examData.lessons }
    }).select().single();
    ...
    for (const q of examData.questions) {
      // Clean up the structure for 'content' JSONB
      // We store the full JSON from the file into the content.
      const qContent = { ...q };
      delete qContent.id;
