...
async function seed() {
  console.log("Starting static seeding for Vietnamese 3 Theme 2...");

  // Load JSON
  const jsonPath = path.join(process.cwd(), 'docs', 'Assement Studio', 'Tieng_Viet_3_Tap1_JSON', 'chu_diem_2_exams.json');
  ...
  const unitTitle = "Chủ điểm 2: Cổng trường rộng mở";
  const unitSlug = "chu_diem_2";
  ...
  for (const examData of data.exams) {
    ...
    const { data: exam, error: exErr } = await supabase.from('exams').insert({
      ...
      metadata_json: { lessons: [examData.title] }
    }).select().single();
