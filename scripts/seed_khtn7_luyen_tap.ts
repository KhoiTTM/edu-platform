...
async function run() {
  ...
  for (const item of khtnExams) {
    ...
    if (!colId) {
      ...
      colId = col.id;
      console.log(`Inserted Collection: ${colTitle} (ID: ${colId})`);
    } else {
      console.log(`✅ Collection already exists: ${colTitle} (ID: ${colId})`);
    }
    ...
    if (!existingExam) {
      ...
      console.log(`Inserted Exam: ${item.title} (ID: ${exam.id})`);
    } else {
      console.log(`✅ Exam already exists: ${item.title} (ID: ${existingExam.id})`);
    }
  }

  console.log('KHTN 7 collections & exams seed successfully completed.');
}
