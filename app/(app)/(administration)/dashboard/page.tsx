...
export default async function DashboardPage() {
  ...
  const [profileRes, statsRes, sessionsRes, quizRes, todayTasks, pendingOldTasks, reflexColRes, reflexCol7Res] = await Promise.all([
    ...
    supabase.from('assessment_collections').select('exams(id)').eq('subject_slug', 'tieng-anh-7').eq('title', 'Phản xạ từ vựng tiếng anh 7').maybeSingle()
  ]);
      ...
      {(grade === 3 || grade === 7) && reflexExamIds.length > 0 && (
        <div className="shrink-0 mb-1 bg-gradient-to-r from-orange-500 to-rose-600 rounded-3xl p-5 flex flex-col md:flex-row items-center justify-between gap-4 shadow-[0_8px_32px_rgba(244,63,94,0.3)] border border-rose-400/20">
          <div className="flex items-center gap-4 text-center md:text-left">
            ...
            <div>
              <h3 className="text-white font-black text-[1.1rem] drop-shadow-sm uppercase tracking-wider">
                {grade === 7 ? "Phản xạ từ vựng tiếng anh 7" : "Phản xạ Từ vựng Cơ bản"}
              </h3>
