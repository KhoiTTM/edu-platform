...
const SubjectCard = ({ subject, grade }: { subject: any, grade: number }) => {
    ...
    return (
        <Link href={`/luyen-tap/${subject.slug}`} className={`group relative block p-8 rounded-3xl border-4 border-slate-900/50 text-white transition-all duration-300 hover:-translate-y-2 ${color.bg} ${color.shadow}`}>
