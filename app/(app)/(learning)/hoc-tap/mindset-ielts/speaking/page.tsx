...
export default async function SpeakingPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <header className="rounded-2xl border border-emerald-900/40 bg-gradient-to-br from-emerald-950/30 via-slate-900/60 to-slate-950 p-6 shadow-xl">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-400 border border-emerald-500/20">
          🗣️ IELTS SPEAKING
        </span>
        <h1 className="mt-3 text-2xl font-extrabold text-white">Luyện Kỹ Năng Nói</h1>
        <p className="mt-1.5 text-sm text-slate-400">
          {lessons.length} buổi luyện nói — bám sát giáo trình Mindset for IELTS Foundation.
        </p>
      </header>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {lessons.map((lesson) => {
          ...
          const card = (
            <div className={`group flex flex-col gap-3 rounded-2xl border p-5 backdrop-blur-md transition-all duration-200 border-slate-800 bg-slate-900/30 hover:border-emerald-500/40 hover:bg-slate-900/60 hover:shadow-lg cursor-pointer`}>
              <div className="flex items-center justify-between">
                <span className="rounded-md bg-emerald-950/60 px-2 py-0.5 text-[10px] font-bold text-emerald-400 border border-emerald-900/40 uppercase">
                  Unit {unit}
                </span>
                <span className="text-[10px] text-slate-500">Buổi {lesson.lesson_index}</span>
              </div>
              <h3 className="text-sm font-semibold text-white leading-snug group-hover:text-emerald-400 transition-colors">
              ...
              <p className="mt-auto text-[11px] text-slate-400 leading-relaxed line-clamp-2">
                {lesson.summary?.replace(/\[.*?\]/g, '').trim() || 'Luyện nói theo chủ đề Unit.'}
              </p>
      ...
      {lessons.length === 0 && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/30 p-10 text-center">
          <p className="text-slate-400">Không có buổi Speaking nào.</p>
        </div>
      )}
    </div>
  );
