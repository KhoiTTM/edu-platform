-- Migration 052: Thêm tên cố định "Thi Học Kỳ 1" cho exam_type IN ('review','final','exam') + units chứa 101
-- Đồng thời xóa overload cũ (5 tham số) để tránh ambiguous call từ trigger

-- 1) Xóa overload cũ nếu còn tồn tại
DROP FUNCTION IF EXISTS public.generate_assessment_title(text, integer, integer, integer[], integer);

-- 2) Cập nhật hàm (6 tham số, p_exam_type có DEFAULT NULL)
...
BEGIN
    -- Tên cố định cho đề thi midterm
    ...
    -- Tên cố định cho đề thi học kỳ
    IF p_exam_type IN ('review', 'final', 'exam') THEN
        IF p_units IS NOT NULL AND 101 = ANY(p_units) THEN
            RETURN 'Thi Học Kỳ 1';
        END IF;
    END IF;
...
-- 3) Cập nhật trigger để luôn truyền exam_type (giống migration 051 nhưng explicit cast)
...
BEGIN
    ...
    IF TG_OP = 'DELETE' THEN
        v_subject := OLD.subject_slug; v_grade := OLD.grade; v_volume := OLD.volume;
        v_units := OLD.units; v_exam_type := OLD.exam_type;
    ELSE
        v_subject := NEW.subject_slug; v_grade := NEW.grade; v_volume := NEW.volume;
        v_units := NEW.units; v_exam_type := NEW.exam_type;
    END IF;
    ...
    WHERE public.assessment_collections.id = ordered.id
      AND (
          ...
          OR public.assessment_collections.title IS DISTINCT FROM
             public.generate_assessment_title(v_subject, v_grade, v_volume, v_units, ordered.new_seq::INTEGER, v_exam_type)
      );
...
-- 4) Áp dụng lại tên cho tất cả collection
UPDATE public.assessment_collections
